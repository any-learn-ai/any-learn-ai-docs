...
**作成: 2026-08-07 / 状態: 設計済み（実装は未着手）**

> ⚠️ **2026-08-08 追記。** このメモを前提に
> [設計書](LLM基盤の非同期サービス化_設計書.md) を書いた。
> その際、ここに書いた事実をコードと dev の実環境で裏取りしたところ、
> **費用試算・`usesLlm` の個数・DLQ の有無・実装の着手状況・stg/prod の状態**に
> 食い違いが見つかった。**設計書の 0 章に訂正表がある。数値を引用する前にそちらを読むこと。**
>
> 特に重要な訂正: 「非同期サービス化で60%減」のうち **58ポイントは
> Lambda のメモリ割り当てを下げるだけで出る**（非同期化そのものの効果は7%）。
> 一方、assessments が Luna を呼べないことの損失は **100人あたり 2,266円**で、
> こちらが最も大きい。
